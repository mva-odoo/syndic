...
class observation_facade(models.Model):
...
class repeir_facade(models.Model):
    _name = 'facade.repair'
...
class repeir_terasse(models.Model):
...
class observation_toiture(models.Model):
...
class repeir_toiture(models.Model):
...
class observation_jardin(models.Model):
...
class repeir_ascensseur(models.Model):
...
class piece_chauffage(models.Model):
...
class repeir_chaudiere(models.Model):
...
class observation_egoutage(models.Model):
...
class repeir_garage(models.Model):
    _name = 'repeir.garage'
    entreprise = fields.Many2one('syndic.supplier', string='Fournisseur')
    date_repeir = fields.Date('Date')
    objet_repeir = fields.Char('Objet de la réparation')
...
class repeir_general(models.Model):
...
class signalitique_immeuble(models.Model):
    ...
    #chauffage
    chauffage_type = fields.Selection([('individuel', 'Individuel'), ('collectif', 'Collectif')],
                                       'Type de chauffage')
    eau_chaude_type = fields.Selection([('individuel', 'Individuel'), ('collectif', 'Collectif')],
                                        'Production eau chaude')
    type_chauffage = fields.Selection([('gaz', 'Gaz'), ('mazoutb', 'Mazout type b'), ('mazoutc', 'Mazout type c')],
                                       'Type')
