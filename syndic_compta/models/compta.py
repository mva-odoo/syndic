...
class SyndicBuilding(models.Model):
    ...
    roulement_product_id = fields.Many2one('syndic.product', 'Produit d ouverture (roulement)', default=_default_prod_roulement)
    reserve_product_id = fields.Many2one('syndic.product', 'Produit d ouverture (resrve)', default=_default_prod_reserve)
    compte_rapporter = fields.Many2one('syndic.pcmn', 'Compte à reporter', default=_default_compte_rapporter)
    report_reserve_compte = fields.Many2one('syndic.pcmn', 'Compte de fond de reserve à reporter', default=_default_report_reserve_compte)
    report_roulement_compte = fields.Many2one('syndic.pcmn', 'Compte de fond de roulement à reporter', default=_default_report_roulement_compte)
    open_report_reserve_compte = fields.Many2one('syndic.product', 'Produit de fond de reserve à reporter pour reouverture', default=_default_open_report_reserve_compte)
    open_report_roulement_compte = fields.Many2one('syndic.product', 'Produit de fond de roulement à reporter pour reouverture', default=_default_open_report_reserve_compte)
